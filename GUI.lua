-- ============================================
--   Garskirtz Premium UI | REBORN (Fix Logic)
-- ============================================
...
-- Clean up old UI
local old = player.PlayerGui:FindFirstChild("GarskirtzPremium") or player.PlayerGui:FindFirstChild("PremiumFarm")
if old then old:Destroy() end

-- ── Shared state (Penting untuk Logic Lama) ──────────────────────────────────
_G.AF_moneyStart   = 0
...
local C = {
    ...
    green   = Color3.fromRGB(60, 170, 110), -- Warna dari Logic lama
    red     = Color3.fromRGB(200, 50, 50),  -- Warna dari Logic lama
...
local statusIcon = Instance.new("TextLabel", statusCard) -- Harus TextLabel karena Logic lama ubah TextColor3
statusIcon.Size = UDim2.new(0, 20, 1, 0); statusIcon.Position = UDim2.new(0, 10, 0, 0)
...
statusText.Size = UDim2.new(1, -40, 1, 0); statusText.Position = UDim2.new(0, 30, 0, 0)
...
bal.Size = UDim2.new(0.5, -20, 0, 60); bal.Position = UDim2.new(0, 20, 0, 95)
...
rate.Size = UDim2.new(0.5, -20, 0, 60); rate.Position = UDim2.new(0.5, 0, 0, 95)
...
btn.BackgroundColor3 = C.card; btn.Text = "START AUTOFARM"; btn.TextColor3 = Color3.new(1,1,1)
btn.Font = Enum.Font.GothamBold; btn.AutoButtonColor = false
Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 10)
local btnStroke = Instance.new("UIStroke", btn)
btnStroke.Color = C.green -- Default color untuk Start
btnStroke.Thickness = 2

-- ── Sinkronisasi Global Variable (_G) untuk Logic Lama ────────────────────────
_G.AF_GUI = {
    statusText = statusText,
    statusIcon = statusIcon, -- Logic lama ganti warna .TextColor3
    btn        = btn,
    btnStroke  = btnStroke,  -- Logic lama ganti warna .Color
    balanceLbl = bal,
    perHourLbl = rate,
    C          = C           -- Menyediakan tabel warna G.C.green dsb
}
...
-- ── Stats Auto-Update (Agar Balance Berubah) ──────────────────────────────────
RunService.Heartbeat:Connect(function()
    ...
    if ls and ls:FindFirstChild("Cash") then
        ...
        if elapsed > 2 then
            rate.Text = "$" .. math.floor(gained/elapsed * 3600) .. "/hr"
        end
    end
end)

print("Garskirtz UI Loaded - Sinkron dengan Logic Lama")
